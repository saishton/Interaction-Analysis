function [Structure] = NumComp_Moments(data,dir_ref)
...
comp_sigma = M1*sqrt(2/pi);
comp_ccdf_rl = raylcdf(X_comp,comp_sigma,'upper');

comp_lnsig = sqrt(log(M2*(M1^-2)));
comp_lnmu = log(M1)-(0.5*comp_lnsig^2);
...
imagefilename = [dir_ref,'/NumberComponents_Moments.png'];
