function [p_vals] = global_pvals_gp(dataLength,k,sigma,theta,Statistics,cut,n,gap)
...
parfor i=1:num_MC
    ...
    for j=1:numData
        ...
        if thisLength >0
            data = gprnd(k,sigma,theta,thisLength,1);
            data = sort(data);
            if cut>0
                data(end-cut+1:end) = [];
            end
            PDF = gppdf(data,k,sigma,theta);
            CDF = gpcdf(data,k,sigma,theta);
